...
# Structure: { lobby_id: { 'host': 'host_id', 'participants': [user_ids] } }
lobbies = {}

@lobby_bp.route('/lobby', methods=['POST'])
def create_lobby():
    """Creates a new lobby given a host_id."""
    ...
    # For demo purposes, we assume the front-end is served from the same domain.
    lobby_url = f"http://localhost:5000/lobby/{lobby_id}"
    return jsonify({'lobby_id': lobby_id, 'lobby_url': lobby_url}), 201
